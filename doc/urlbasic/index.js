(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["urlbasic"] = factory();
	else
		root["urlbasic"] = factory();
})(this, function() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	(function (CommandType) {
	    CommandType[CommandType["print"] = 0] = "print";
	    CommandType[CommandType["locate"] = 1] = "locate";
	    CommandType[CommandType["cls"] = 2] = "cls";
	    CommandType[CommandType["delay"] = 3] = "delay";
	    CommandType[CommandType["if"] = 4] = "if";
	    CommandType[CommandType["end"] = 5] = "end";
	    CommandType[CommandType["else"] = 6] = "else";
	    CommandType[CommandType["elif"] = 7] = "elif";
	    CommandType[CommandType["for"] = 8] = "for";
	    CommandType[CommandType["while"] = 9] = "while";
	    CommandType[CommandType["let"] = 10] = "let";
	})(CommandType || (CommandType = {}));
	var maxExecCommandCount = 100;
	var maxScreenPos = 50;
	...
	var pc;
	var cursorPos;
	var isBackToScopeFirst;
	var vars;
	var delayCount;
	var isRunning;
	var requestAnimationFrameHandler;
	var parsingCommand;
	var parsingPc;
	function init(code) {
	    ...
	    vars = { mod: function (a, b) { return a % b; }, true: (0 == 0), false: (0 == 1) };
	    delayCount = 0;
	...
	function parseCode(code) {
	    commands = _.map(code.split(':'), function (c, i) {
	        parsingCommand = c;
	        parsingPc = i;
	        var typeStr = c;
	        var args = [];
	        ...
	        if (ai > 0) {
	            ...
	            args = _.map(c.substr(ai).split(';'), function (a) { return a.trim(); });
	        }
	        typeStr = typeStr.trim();
	        var type;
	        var ck = _.findKey(CommandType, function (cStr) { return _.startsWith(cStr, typeStr); });
	        if (ck != null) {
	            type = Number(ck);
	        }
	        ...
	        return { type: type, args: args, str: c.trim() };
	    });
	}
	function update() {
	    if (!isRunning) {
	    ...
	    for (var i = 0; i < maxExecCommandCount; i++) {
	...
	function execCommand(cmd) {
	    switch (cmd.type) {
	        ...
	        case CommandType.for:
	            ...
	            var v = parseExpression(cmd.args[0]).assignVar;
	            var to = execExpression(cmd.args[1]);
	            var step = cmd.args.length < 3 ? 1 : execExpression(cmd.args[2]);
	        ...
	        case CommandType.if:
	            if (isBackToScopeFirst) {
	                ...
	                var cond = execExpression(cmd.args[0]);
	                if (!cond) {
	                    breakScope(true);
	                }
	            }
	...
	function breakScope(isBreakToElse) {
	    if (isBreakToElse === void 0) { isBreakToElse = false; }
	    var nest = 0;
	    for (var i = 0; i < maxExecCommandCount; i++) {
	        pc++;
	        if (pc >= commands.length) {
	...
	function parseExpression(exp) {
	    ...
	    return { assignVar: assignVar, expStr: expStr };
	}
	function parseArray(exp, i) {
	    if (i === void 0) { i = 0; }
	    var bi = exp.indexOf('[', i);
	    if (bi < 0) {
	        return exp;
	    }
	    var ei = exp.indexOf(']', i);
	    if (ei < 0) {
	        throw 'invalid array';
	    }
	    var idxStr = exp.substring(bi + 1, ei);
	    var idx = execExpression(idxStr);
	    var prevStr = exp.substring(0, bi);
	    var arrayStr = prevStr + "__" + String(idx);
	    return parseArray(arrayStr + exp.substring(ei + 1), arrayStr.length);
	}
	function parseFunction(exp) {
	    var regExp = /(^|[^a-zA-Z0-9_])(k|ke|key|s|sc|scr|scre|scree|screen)\((.*)\)/;
	    var fm = exp.match(regExp);
	    if (fm == null) {
	        return exp;
	    }
	    var name = fm[2];
	    var arg = execExpression(fm[3]);
	    var funcStr;
	    if (name[0] === 'k') {
	    ...
	    var parsed = exp.replace(regExp, function (m, p1, p2, p3, o, s) { return p1 + funcStr; });
	    return parseFunction(parsed);
	}
	function execExpression(exp) {
	    var pe = parseExpression(exp);
	    var value = Parser.parse(pe.expStr).evaluate(vars);
	    if (pe.assignVar != null) {
	        vars[pe.assignVar] = value;
	    }
	    return value;
	}
	function indexOfAssign(s, i) {
	    if (i === void 0) { i = 0; }
	    var ei = s.indexOf('=', i);
